    ...
    -- SQL 문법
		-- DDL	[Data Definition L
			-- create	스키마 혹은 테이블 생성
				-- create table 테이블명(필드명 자료형 속성명, 필드명2 자료형 속성명);
            -- alter	스키마 혹은 테이블 수정
            -- drop		스키마 혹은 테이블 삭제
...
-- select --
	-- 예1) select 필드명1,필드명2,필드명3 from 테이블명
select empno, ename, sal from emp;
-- * (와일드카드 = 모든 필드)
select * from emp;
select empno, ename,job,mgr,hiredate,sal,comm,deptno from emp;
-- 예2) 필드 별칭[별명] as 화면에 표시되는 이름
select empno as 사원번호 from emp;
-- 예3) distinct : 필드의 데이터 중복제거
select distinct job as 직업 from emp;
-- 예4) order by : 정렬 [ order by 필드명 asc/desc, 필드명2 asc/desc ]
	-- 하위 정렬은 상위 정렬이 동일한 경우가 있을경우 적용
select ename as 이름, sal as 월급 from emp order by sal asc;
	-- 월급 기준으로 오름차순 하고 이름과 월급 검색
select deptno as 부서코드, ename as 이름, sal as 월급 from emp order by DEPTNO asc, sal desc;
	-- 부서코드 기준으로 오름차순 하고 동일한 부서코드가 있을경우 동일한 부서코드내에서 월급을 내림차순으로 정렬후 검색
...
select * from emp where sal between 1000 and 3000;		/* 월급이 1000~3000인*/
select * from emp where sal >=1000 and sal<=3000;

select * from emp where sal not between 1000 and 3000;	/* 월급이 1000~3000 제외한*/
select * from emp where sal <1000 or sal>3000;
	-- between and vs and
select * from emp where hiredate between '81/1/1' and '82/12/31';
select * from emp where hiredate >='81/1/1' and hiredate <= '82/12/31';
	-- in vs or
...
select * from emp where ename like 'S_';	-- S로 시작하는 두글자 이름 검색
select * from emp where ename like '%S';	-- S로 끝나는 이름 검색
select * from emp where ename like '%S%';	-- S가 포함된 이름 검색
select * from emp where ename like '_S%';	-- 두번째 글자가 S인 이름 검색

	-- null
select * from emp where comm = NULL;	-- null 비교연산자 불가능
select * from emp where comm is NULL;	-- null 찾기
select * from emp where comm is not null;	-- null이 아닌것 찾기

	-- 문자열 함수 [sql 내장 함수 = 기본적으로 제공해주는 미리 만들어진 함수]
		-- upper(필드명) : 해당 필드의 데이터 대문자로 변환
        -- lower(필드명) : 해당 필드의 데이터 소문자로 변환
		-- substr('문자열','시작인덱스','개수') : 해당 문자열을 시작인덱스부터 개수만큼 표시
        -- length('문자열') : 문자열 길이 [ 한글 : 3바이트 / 영어 : 1바이트 ]
        -- instr('문자열','찾을문자') : 해당 문자열내 찾을 문자의 인덱스순서 번호
        -- trim('문자열') : 해당 문자열의 앞뒤 공백 제거
        -- rtrim('문자열') : 해당 문자열의 오른쪽 공백 제거
        -- ltrim('문자열') : 해당 문자열의 왼쪽 공백 제거
        -- replace('문자열','기존문자','새로운문자') : 해당 문자열의 기존문자를 새로운문자로 변경
select upper(ename), lower(ename) from emp;
	-- 검색 엔진 대소문자 제거
select * from emp where ename = 'scott';	-- mysql 대소문자 구분 x  	vs 	oracle 대소문자 구분 o
	-- dual : 임시 테이블 -> 연산 결과만 임시로 확인할 수 있는 테이블 [ 영구저장 X ]
select substring('대한민국', 1,3) from dual;
	-- length()
select length('대한민국') from dual;	-- 결과 : 12	[ 한글 : 3바이트 ]
select length('smith') from dual;	-- 결과 : 5	[ 영어 : 1바이트 ]
	-- instr()
select instr('대한민국','대') from dual;	-- 결과 : 1
select instr('smith','i') from dual;	-- 결과 : 3
	-- trim()
select rtrim('    대      한 민               국      ') from dual;
select trim( '*' from '  대      한 민               국      ') from dual;
